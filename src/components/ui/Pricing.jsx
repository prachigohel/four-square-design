        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-10 max-w-7xl mx-auto items-stretch">
          {plans.map((plan, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className={`group relative flex flex-col rounded-[2.5rem] overflow-hidden backdrop-blur-2xl transition-all duration-500 hover:-translate-y-3 h-full ${
                plan.recommended 
            ...
            >
              ...
              <div className={`px-8 lg:px-10 flex flex-col items-center text-center ${!plan.recommended ? 'pt-16' : 'pt-20'} pb-12 relative z-0`}>
                ...
                <h3 className={`text-3xl md:text-4xl font-serif italic mb-3 ${plan.recommended ? 'text-white' : 'text-slate-900 dark:text-white'}`}>
                  {plan.name}
                </h3>
                ...
                <div className="flex items-start justify-center mb-2">
                  <span className={`text-2xl font-bold mt-2 mr-1 ${plan.recommended ? 'text-amber-400' : 'text-slate-400 dark:text-slate-500'}`}>$</span>
                  <span className={`text-6xl md:text-7xl font-sans font-black tracking-tighter ${plan.recommended ? 'text-white' : 'text-slate-900 dark:text-white'}`}>
                    {plan.price}
                  </span>
                </div>
                ...
                <Link
                  to="/request"
                  className={`inline-flex items-center justify-center gap-3 w-full py-4 rounded-full font-bold text-xs uppercase tracking-[0.2em] transition-all duration-500 ${
                    plan.recommended
              ...
              <div className={`px-8 lg:px-10 pb-12 pt-10 border-t flex-grow ${
                plan.recommended 
                ...
                <p className={`text-[10px] font-bold uppercase tracking-[0.3em] mb-8 ${plan.recommended ? 'text-white' : 'text-slate-900 dark:text-white'}`}>
                  {index === 0 ? 'Everything Included:' : index === 1 ? 'All in Kitchen Plan plus:' : 'All in Full House Plan plus:'}
                </p>
